...
// ************************************************************************************************************
// I2C Compass MPU9250_MAG
// ************************************************************************************************************
// I2C adress: 0x0C (7bit)
// ************************************************************************************************************
#if defined(MPU9250_MAG)
#define MAG_ADDRESS 0x0C
#define MAG_DATA_REGISTER 0x03
// AK8963 registers
...
void Mag_init() {
  delay(100);
  i2c_writeReg(MAG_ADDRESS, AK8963_CNTL1, AK8963_PWR_DOWN); // Power down
  delay(100);
  i2c_writeReg(MAG_ADDRESS, AK8963_CNTL2, 0x01); // Reset

  // set AK8963 to Power Down
  i2c_writeReg(MAG_ADDRESS, AK8963_CNTL1, AK8963_PWR_DOWN);
  delay(100); // long wait between AK8963 mode changes

  // set AK8963 to FUSE ROM access
  i2c_writeReg(MAG_ADDRESS, AK8963_CNTL1, AK8963_FUSE_ROM);
  delay(100); // long wait between AK8963 mode changes
  ...
  i2c_writeReg(MAG_ADDRESS, 0x37, 0x02);  // Set By Pass mode

  // set AK8963 to 16 bit resolution, 100 Hz update rate
  i2c_writeReg(MAG_ADDRESS, AK8963_CNTL1, AK8963_CNT_MEAS2);
  delay(100); // long wait between AK8963 mode changes
  /*i2c_writeReg(MAG_ADDRESS,0x37,0x02);    // Set By Pass mode
    i2c_writeReg(MAG_ADDRESS,0x0a,0x016);   // Request Continous magnetometer measurements in 16 bits
  */
  delay(100);
}
...
void Device_Mag_getADC() {
  ...
  MAG_ORIENTATION( ((rawADC[1] << 8) | rawADC[0]) ,
                   ...
                   ((rawADC[5] << 8) | rawADC[4]) );
  //Start another meassurement
  i2c_writeReg(MAG_ADDRESS, 0x0a, 0x016);
}
#endif
